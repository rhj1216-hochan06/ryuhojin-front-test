...
- 공공데이터포털 실제 OpenAPI 검색, 응답 정규화, 취소, 재시도, 페이지네이션 처리
...
`#/api-playground` 페이지는 한국관광공사 국문 관광정보 서비스_GW의 키워드 검색 API를 직접 호출합니다. 검색어, 응답 상태, 요청 ID, 응답 시간, 페이지네이션 결과, 이미지 fallback을 한 화면에서 확인할 수 있고, 요청 취소와 마지막 요청만 화면에 반영하는 방어 로직을 함께 제공합니다.
...
- API 테스트 페이지에서 관광정보 키워드 검색, 이미지 fallback, 응답 시간, 취소, 재시도, 스크롤 하단 자동 조회 확인
- 모바일에서 카드, 차트, timeline, table의 가로 스크롤과 텍스트 겹침 확인

## 포트폴리오 설명 포인트

- mock API 응답을 직접 렌더링하지 않고 hook과 view model로 가공합니다.
- axios 공통 설정, API 호출부, React Query hook, 화면 hook, UI를 분리해 실제 공공데이터 검색과 실패/취소/재시도/페이지네이션 상태를 명확히 처리합니다.
