...
메인 페이지는 해결하려는 화면 문제, 구현으로 보여주는 프론트엔드 역량, 기능별 데모 진입점을 나눠 보여주고, 복잡한 화면은 hash route 기반의 별도 페이지로 분리했습니다. CRA 정적 배포에서 새로고침 404 위험을 줄이기 위해 별도 라우팅 패키지를 추가하지 않고 `#/charts`, `#/data-grid`, `#/timeline` 구조를 사용합니다.
